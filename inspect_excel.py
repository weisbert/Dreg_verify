...
用法:
    python inspect_excel.py <excel路径>
        ...
        [--maxlen N]          单元格内容最大显示长度 (默认 80, logic 的表达式列不截断)

脱敏说明:
  - 默认【不脱敏】，导出原文；请你自己过目后再决定要不要手动删改。
  - --anon-signals 会把字母词元替换成 w1/w2/...，但【保留】下划线、方括号、位宽数字、
    分隔符与表达式运算符；logic 的真值表达式列(L) 始终保持原文（那是符号逻辑，不敏感且是核心）。
...
def transform_cell(sheet_name, col_letter, value, args):
    """根据开关与列位置返回（可能脱敏后的）原始值（仍是 python 对象/字符串）。"""
...
def dump_sheet(out, ws, sheet_name, args):
    ...
    # 前几行原始（含列字母），帮我判断表头是否跨多行
    out.append("--- 前 5 行原文 ---")
    for ri in range(min(5, len(rows))):
        cells = []
        for c in range(max_col):
            letter = get_column_letter(c + 1)
            v = transform_cell(sheet_name, letter,
                              rows[ri][c] if c < len(rows[ri]) else None, args)
            sv = cell_str(v, args.maxlen)
            if sv != "":
                cells.append("%s=%s" % (letter, sv))
        out.append("[row %d] %s" % (ri + 1, " | ".join(cells)))
    ...
    # logic 页：额外做列画像 + 若干行竖排完整内容
    if sheet_name.lower() == "logic":
        dump_logic_extra(out, rows, max_col, sheet_name, args)


def dump_logic_extra(out, rows, max_col, sheet_name, args):
    ...
    out.append("--- [logic] 前 12 个数据行竖排 (表达式列 L 不截断) ---")
    header = rows[0]
    shown = 0
    for ri in range(1, len(rows)):
        if shown >= 12:
...
def main():
    ...
    out = []
    out.append("Dreg Excel 结构导出")
    out.append("文件: %s" % os.path.basename(args.excel))
    out.append("全部 sheet (%d): %s" % (len(all_sheets), all_sheets))
    out.append("本次导出 sheet: %s" % sheets)
    if missing:
        out.append("⚠ 指定但不存在的 sheet: %s" % missing)
    out.append("开关: anon_signals=%s, mask_owners=%s, rows=%s, maxlen=%s"
               % (args.anon_signals, args.mask_owners, args.rows, args.maxlen))

    for sn in sheets:
        ws = wb[sn]
        dump_sheet(out, ws, sn, args)
