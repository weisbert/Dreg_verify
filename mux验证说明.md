...
| 覆盖度 | mux 测试内容 | tsensor 类 mux（8 case 带 x 位）的 T 数 |
|--------|------------|--------------------------------------|
| 精简 | 每 case 一个控制值（don't-care 位取 0）+ 1 个另一条路径测试 | 8+1 = 9 |
| 全面 / 穷举 | 每 case × 每个 don't-care 位取值 + 另一条路径测试 | 16+1 = 17 |

**最后一个 T（另一条路径）**：控制信号切到 local 路径（模式位=1 + 本地寄存器=case 值），
