...
class WorkbenchStateProto(Protocol):
    ...
    def reset_config_state(self) -> bool:
        """C-192「先清空再照单恢复」：导入【完整配置】前把可编辑状态清回出厂态。

        三套诊断配置 + `include_risky` + 覆盖度三层 + 各范围的编辑 / mux 数据值 / 勾选，
        **不碰已载入的 wb / provider / 清单模型**。导入是「加载这份工作状态」而不是
        「往当前状态上叠加」—— 不清就会得到两份配置的混合物（C4-int additive）。"""
        ...
    # ── 持久化 / 偏好 ──
...
class TruthModelProto(Protocol):
    ...
    def is_mux_auto_data_cell(self, r: int, c: int) -> bool:
        """这一格是不是【自动生成列】的 mux 数据值格（C-110：只能整表一起改）。

        C3-int additive：`truth/io.paste_plan` 按它把这种格子分进「要整表改」那一桶
        （逐格粘会让后一格覆盖前一格，屏幕值与导出值当场分家 —— C-112）。判据只在 model。"""
        ...
    def undo_stack(self): ...
