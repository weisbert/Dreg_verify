...
| C-288 | 覆盖度叠加：把当前向量集实际走到的 mux 支点亮、没被驱动过的支涂灰（假绿可视化） | 无 | sigflow.build_graph | 新增 | 主控补位: 覆盖度叠加（点亮走到的 mux 支 / 没驱动过的涂灰）排到 v2 之后单开任务（执行计划 §5 待拍板 3 默认值）；V2Spec §4「深度上限 6 层 / 约 60 节点」给了预留空间 | | 已落点(补位) | 方案§3-轨1-二期; TASK-015; 计划§5-3; 原 status: 待拍板(默认=v2 之后单开任务) |
| C-289 | 多信号总览图 + PDF 导出 | 无 | — | 新增 | 主控补位: 多信号总览图 + PDF 导出排到 v2 之后单开任务（执行计划 §5 待拍板 3 默认值） | | 已落点(补位) | TASK-015; 计划§5-3; 原 status: 待拍板(默认=v2 之后单开任务) |
